...
def generateAudioLabelsDictionary(bucketed=False):
    ...
    for index, row in df.iterrows():
        if 'Book' in row['ID_TASK']:
            key = row['ID_TASK'] + '.m4a'
        elif 'Block' in row['ID_TASK']:
            key = row['ID_TASK'] + '.mp3'
        else:
            print(f"error: {row['ID_TASK']} is not book or block")
            continue
...
if __name__ == "__main__":
    generateAudioLabelsDictionary(bucketed=True)